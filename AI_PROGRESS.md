...
- The first authenticated production export smoke test exposed a CommonJS/ESM interop defect masked by the local `tsx` runner: `exceljs` has named `Workbook` exports in the deployed runtime. Changed the import to a namespace import, rebuilt successfully, and is awaiting the corrected deployment and XLSX download validation.
