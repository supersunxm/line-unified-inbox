...
- Scope guard: preserve coordinator priorities, LINE mappings, nickname formatting, tagging, product logic, profile paths, and authenticated profile contents. No production deployment or profile cleanup is authorized.
...
- Prisma migration status shows a pre-existing failed migration blocking pending local migrations. No migration recovery, schema mutation, profile cleanup, commit, push, deployment, or production access was performed.
- Next action: user review of the uncommitted local patch. The pre-existing untracked `docs/executive-guide/` directory remains untouched.
