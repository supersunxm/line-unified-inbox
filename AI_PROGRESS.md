...
- The route sequence `/chats` → `/replymessage` → `/store-360` → `/` → `/chats` rendered and remained responsive. Browser Performance/Memory/Long Task APIs are unavailable through the current automation surface, so no heap or Long Task values are claimed. No production deployment or production mutation has been performed yet.
