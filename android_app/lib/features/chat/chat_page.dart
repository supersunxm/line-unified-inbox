...
class _ChatPageState extends State<ChatPage> {
  ...
  Future<void> _pickImage() async {
    try {
      ...
      if (send == true) await _sendImage(bytes, picked.name);
    } catch (_) { if (mounted) setState(() => _error = 'Unable to choose this image'); }
  }
  Future<void> _sendImage(Uint8List bytes, String filename, {PendingImage? existing}) async {
    ...
    _scrollToBottom();
    try { await widget.repository.sendImage(widget.conversationId, pending.bytes, pending.filename, pending.key); if (mounted) { setState(() => _pendingImages.remove(pending)); _load(); } }
    on ApiException catch (error) { if (mounted) setState(() { pending.state = ReplyState.failed; _error = error.message; }); }
