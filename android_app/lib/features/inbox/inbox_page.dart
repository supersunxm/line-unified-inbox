...
class InboxPage extends StatefulWidget {
  const InboxPage({super.key, required this.repository, required this.onOpen, required this.onProfile, this.events});
...
class _InboxPageState extends State<InboxPage> {
  ...
  @override void initState() { super.initState(); _scroll.addListener(_onScroll); _eventsSubscription = widget.events?.listen((event) { final type = event['type']; if (mounted && !_loadingMore && (type == 'connected' || type == 'message.created' || type == 'message.media.updated' || type == 'conversation.updated')) _load(reset: true); }); _load(reset: true); }
  @override void dispose() { _eventsSubscription?.cancel(); _scroll.dispose(); super.dispose(); }
