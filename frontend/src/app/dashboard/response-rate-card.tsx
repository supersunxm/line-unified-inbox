    ...
    <div className="app-card p-5 rounded-2xl border border-slate-200/80 dark:border-slate-800/80 bg-white dark:bg-slate-900 shadow-2xs font-tabular">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-slate-800/80">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{t.targetNotice}</p>
        </div>
        <div className="flex items-center gap-3 text-xs">
          <div className="px-3 py-1.5 rounded-xl bg-slate-50 dark:bg-slate-800/50 text-slate-700 dark:text-slate-200 border border-slate-100 dark:border-slate-800/60">
            <span className="text-slate-500 dark:text-slate-400">{t.avgTime}: </span>
            <span className="font-bold text-emerald-600 dark:text-emerald-400">{formatTime(analytics.avgResponseMinutes, t)}</span>
          </div>
          <div className="px-3 py-1.5 rounded-xl bg-slate-50 dark:bg-slate-800/50 text-slate-700 dark:text-slate-200 border border-slate-100 dark:border-slate-800/60">
            <span className="text-slate-500 dark:text-slate-400">{t.medianTime}: </span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-4">
        {/* Bucket 1: < 4h */}
        <div className="p-3.5 rounded-xl border border-emerald-200/60 dark:border-emerald-900/40 bg-emerald-50/40 dark:bg-emerald-950/20">
          ...
          <div className="mt-2 text-xl font-bold text-emerald-700 dark:text-emerald-300">{b.under4h}</div>
          <div className="w-full h-1.5 bg-emerald-200/60 dark:bg-emerald-950 rounded-full mt-2 overflow-hidden">
            <div className="h-full bg-emerald-500 rounded-full" style={{ width: `${p1}%` }} />
          </div>
        </div>

        {/* Bucket 2: 4-12h */}
        <div className="p-3.5 rounded-xl border border-teal-200/60 dark:border-teal-900/40 bg-teal-50/40 dark:bg-teal-950/20">
          <div className="flex items-center justify-between">
          ...
          <div className="mt-2 text-xl font-bold text-teal-700 dark:text-teal-300">{b.between4and12h}</div>
          <div className="w-full h-1.5 bg-teal-200/60 dark:bg-teal-950 rounded-full mt-2 overflow-hidden">
        ...
        <div className="p-3.5 rounded-xl border border-amber-200/60 dark:border-amber-900/40 bg-amber-50/40 dark:bg-amber-950/20">
          ...
          <div className="mt-2 text-xl font-bold text-amber-700 dark:text-amber-300">{b.between12and24h}</div>
          <div className="w-full h-1.5 bg-amber-200/60 dark:bg-amber-950 rounded-full mt-2 overflow-hidden">
            <div className="h-full bg-amber-500 rounded-full" style={{ width: `${p3}%` }} />
          </div>
        </div>

        {/* Bucket 4: > 24h */}
        <div className="p-3.5 rounded-xl border border-rose-200/60 dark:border-rose-900/40 bg-rose-50/40 dark:bg-rose-950/20">
          <div className="flex items-center justify-between">
          ...
          <div className="mt-2 text-xl font-bold text-rose-700 dark:text-rose-300">{b.over24h}</div>
          <div className="w-full h-1.5 bg-rose-200/60 dark:bg-rose-950 rounded-full mt-2 overflow-hidden">
