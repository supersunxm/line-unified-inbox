...
import { responseDrilldownHref } from "./response-utils";
...
function MetricCard({ label, value, helper, comparison, icon, tone = "blue", href }: { label: string; value: string; helper?: string; comparison?: string; icon: string; tone?: "blue" | "green" | "purple" | "red" | "amber"; href?: string }) {
  const toneClass = {
  ...
  const className = surfaceClass("min-h-[138px] p-4 sm:p-5") + (href ? " block cursor-pointer transition-colors hover:border-[var(--app-accent)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--app-accent)]/40" : "");
  return href ? <Link href={href} aria-label={label + " — open Response Analysis"} className={className}>{body}</Link> : <article className={className}>{body}</article>;
}
...
function ResponseBars({ summary, storeId, from, to }: { summary: StoreInsightsSummary; storeId: string; from: string; to: string }) {
  const response = summary.response;
  const metrics: Array<[string, typeof response.repliedWithin15Minutes, StoreInsightsResponseSegment]> = [["Within 15 minutes", response.repliedWithin15Minutes, "within-15m"], ["Within 1 hour", response.repliedWithin1Hour, "within-1h"], ["Within 24 hours", response.repliedWithin24Hours, "within-24h"], ["Unanswered", response.unanswered, "unanswered"]];
  return (
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1.05fr)_minmax(230px,0.95fr)]">
      <div className="space-y-4">{metrics.map(([label, metric, segment]) => { const width = metric.percentage === null ? 0 : Math.min(100, Math.max(metric.percentage * 100, metric.count > 0 ? 3 : 0)); const isUnanswered = label === "Unanswered"; return <Link key={label} href={responseDrilldownHref(storeId, from, to, segment)} aria-label={label + " — open Response Analysis"} className="group block rounded-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--app-accent)]/40"><div className="mb-1.5 flex items-center justify-between gap-3 text-xs"><span className="text-[var(--app-text-secondary)] group-hover:text-[var(--app-text-primary)]">{label}</span><span className="font-semibold tabular-nums text-[var(--app-text-primary)]">{metric.count.toLocaleString()} · {formatPercentage(metric.percentage)} <span aria-hidden="true" className="text-[var(--app-accent)]">→</span></span></div><div className="h-2 overflow-hidden rounded-full bg-[var(--app-surface-subtle)]"><div className={"h-full rounded-full transition-[width] " + (isUnanswered ? "bg-[var(--app-warning)]" : "bg-[var(--app-accent)]")} style={{ width: width + "%" }} /></div></Link>; })}{!response.available && <p className="rounded-xl bg-[var(--app-warning-soft)] p-3 text-xs leading-5 text-[var(--app-warning)]">Response percentages are unavailable because some outbound messages do not identify a human responder or automated response.</p>}</div>
...
function BusinessPerformance({ summary }: { summary: StoreInsightsSummary }) {
  ...
  const maxCount = Math.max(1, ...trend.flatMap((point) => [point.customers, point.salesTaggedCustomers]));
  const barStep = trend.length > 0 ? 590 / trend.length : 590;
  const barWidth = Math.max(3, Math.min(16, barStep * 0.34));
  const countHeight = (value: number) => value / maxCount * 56;
  const replyStep = trend.length > 1 ? 590 / (trend.length - 1) : 0;
  const replyY = (value: number) => 70 - Math.min(1, Math.max(0, value)) * 48;
  const replyPoints = trend.reduce<string[]>((points, point, index) => {
    if (point.replyRate === null) return points;
    points.push(`${42 + index * replyStep},${replyY(point.replyRate)}`);
    return points;
  }, []).join(" ");
  const firstDate = trend[0]?.date;
  const middleDate = trend[Math.floor((trend.length - 1) / 2)]?.date;
  const lastDate = trend[trend.length - 1]?.date;
  const dateLabels = [firstDate, middleDate, lastDate].filter((value, index, values): value is string => Boolean(value) && values.indexOf(value) === index);
  const periodReplyRate = summary.response.repliedWithin24Hours.percentage;
  ...
  return (
    <section className={surfaceClass("flex min-h-[360px] flex-col p-4 sm:p-5")} aria-labelledby="business-performance-title">
      ...
      <div className="mt-1 flex flex-1 flex-col">
        <div className="flex items-start justify-between gap-3">
          <div><h3 className="text-xs font-semibold text-[var(--app-text-primary)]">Daily activity</h3><p className="mt-1 text-[11px] text-[var(--app-text-tertiary)]">Bangkok time · selected period</p></div>
          <span className="shrink-0 text-[11px] font-medium text-[var(--app-text-tertiary)]">{summary.period.from} → {summary.period.to}</span>
        </div>
        {!hasTrend ? <div className="mt-4 flex h-44 items-center justify-center rounded-xl bg-[var(--app-surface-subtle)] text-xs text-[var(--app-text-tertiary)]">Daily trend data is not available for this period.</div> : <div className="mt-3 space-y-2.5">
          <div className="rounded-xl border border-[var(--app-border-subtle)] bg-[var(--app-surface-subtle)] px-3 py-2.5">
            <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-1">
              <div><p className="text-[11px] font-semibold text-[var(--app-text-primary)]">Customers & sales-tagged</p><p className="mt-0.5 text-[9px] text-[var(--app-text-tertiary)]">Daily counts · same scale</p></div>
              <div className="flex flex-wrap items-center gap-3 text-[9px] text-[var(--app-text-secondary)]"><span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-[var(--store360-chart-customers)]" />Customers</span><span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-[var(--store360-chart-sales)]" />Sales-tagged</span></div>
            </div>
            <div className="mt-1 h-[72px]"><svg className="h-full w-full text-[var(--app-text-tertiary)]" viewBox="0 0 656 82" preserveAspectRatio="none" role="img" aria-label="Daily customers and sales-tagged customers"><g opacity="0.7">{[20, 45, 70].map((y) => <line key={y} x1="42" x2="632" y1={y} y2={y} stroke="currentColor" strokeOpacity="0.16" strokeWidth="1" />)}</g><g>{trend.map((point, index) => { const x = 42 + index * barStep; const customers = countHeight(point.customers); const sales = countHeight(point.salesTaggedCustomers); return <g key={point.date}><rect x={x} y={70 - customers} width={barWidth} height={customers} rx="2" fill="var(--store360-chart-customers)" fillOpacity="0.82"><title>{formatTrendDate(point.date)} · {point.customers.toLocaleString()} customers</title></rect><rect x={x + barWidth + 1} y={70 - sales} width={barWidth} height={sales} rx="2" fill="var(--store360-chart-sales)" fillOpacity="0.88"><title>{formatTrendDate(point.date)} · {point.salesTaggedCustomers.toLocaleString()} sales-tagged customers</title></rect></g>; })}</g></svg></div>
            {dateLabels.length > 0 && <div className="mt-0.5 flex items-center justify-between text-[9px] text-[var(--app-text-tertiary)]">{dateLabels.map((date) => <span key={date}>{formatTrendDate(date)}</span>)}</div>}
          </div>
          <div className="rounded-xl border border-[var(--app-border-subtle)] bg-[var(--app-surface-subtle)] px-3 py-2.5">
            <div className="flex items-start justify-between gap-3"><div><p className="flex items-center gap-1.5 text-[11px] font-semibold text-[var(--app-text-primary)]"><span className="h-2 w-2 rounded-full bg-[var(--store360-chart-reply)]" />Reply within 24h</p><p className="mt-0.5 text-[9px] text-[var(--app-text-tertiary)]">Daily percentage · separate 0–100% scale</p></div>{periodReplyRate !== null && <span className="shrink-0 text-[11px] font-bold tabular-nums text-[var(--store360-chart-reply)]">Period {formatPercentage(periodReplyRate)}</span>}</div>
            {replyPoints ? <div className="mt-1 h-[58px]"><svg className="h-full w-full text-[var(--app-text-tertiary)]" viewBox="0 0 656 82" preserveAspectRatio="none" role="img" aria-label="Daily reply rate within 24 hours"><g opacity="0.72"><line x1="42" x2="632" y1="22" y2="22" stroke="currentColor" strokeOpacity="0.14" strokeWidth="1" /><line x1="42" x2="632" y1="46" y2="46" stroke="currentColor" strokeOpacity="0.14" strokeWidth="1" /><line x1="42" x2="632" y1="70" y2="70" stroke="currentColor" strokeOpacity="0.14" strokeWidth="1" /></g><text x="0" y="25" fill="currentColor" fontSize="9">100%</text><text x="8" y="49" fill="currentColor" fontSize="9">50%</text><text x="16" y="73" fill="currentColor" fontSize="9">0%</text><polyline points={replyPoints} fill="none" stroke="var(--store360-chart-reply)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />{trend.map((point, index) => point.replyRate === null ? null : <circle key={point.date} cx={42 + index * replyStep} cy={replyY(point.replyRate)} r="3" fill="var(--store360-chart-reply)"><title>{formatTrendDate(point.date)} · {formatPercentage(point.replyRate)} replied within 24h</title></circle>)}</svg></div> : <div className="mt-2 flex h-[58px] items-center justify-center text-[10px] text-[var(--app-text-tertiary)]">Reply-rate trend is not available for this period.</div>}
          </div>
        </div>}
      </div>
    </section>
    ...
    <AppShell currentSection="store-360" authUser={authUser} text={{ appName: "OPPO LINE OA Monitor", searchPlaceholder: "Search customers, stores, or messages" }} language={language} changeLanguage={setLanguage} searchText={searchText} setSearchText={setSearchText} logout={logout} showGlobalHeader={false} isLoading={loading && !summary} apiError={error} loadApplicationData={() => { void loadSummary(); void loadCustomerVoice(); void loadConversations(); }}>
      <PageContainer variant="wide" className="store360-workspace min-w-0 bg-[var(--app-bg)]">
        {!stores.length && !loading ? <div className={surfaceClass("p-8 text-center text-sm text-[var(--app-text-secondary)]")}>No authorized stores available.</div> : loading && !summary ? <Store360Skeleton /> : summary ? <div className="space-y-5">
          ...
          <section aria-labelledby="store-kpis-title"><div className="sr-only"><h2 id="store-kpis-title">Store performance KPIs</h2></div><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-6"><MetricCard label="Customers" value={formatNumber(summary.customers)} helper="Unique customers with inbound activity" comparison={comparisonCountDelta(summary.customers, summary.comparison?.customers)} icon="customers" tone="blue" /><MetricCard label="Reply within 24h" value={summary.response.available ? formatPercentage(summary.response.repliedWithin24Hours.percentage) : "No data available"} helper={summary.response.available ? summary.response.repliedWithin24Hours.count.toLocaleString() + " conversations" : "Human/bot attribution incomplete"} comparison={comparisonPercentagePoints(summary.response.repliedWithin24Hours.percentage, summary.comparison?.response.repliedWithin24Hours.percentage)} icon="reply" tone="green" href={responseDrilldownHref(activeStoreId, from, to, "within-24h")} /><MetricCard label="Median response time" value={summary.response.available ? formatDuration(summary.response.medianFirstResponseSeconds) : "No data available"} helper="First valid human response" comparison={comparisonResponseTime(summary.response.medianFirstResponseSeconds, summary.comparison?.response.medianFirstResponseSeconds)} icon="clock" tone="blue" href={responseDrilldownHref(activeStoreId, from, to, "all")} /><MetricCard label="Sales-tagged customers" value={formatNumber(summary.sales.salesTaggedCustomers)} helper={summary.sales.salesTaggedCustomerPercentage === null ? "Percentage unavailable" : formatPercentage(summary.sales.salesTaggedCustomerPercentage) + " of customers"} comparison={comparisonCountDelta(summary.sales.salesTaggedCustomers, summary.comparison?.sales.salesTaggedCustomers)} icon="sales" tone="purple" /><MetricCard label="Unanswered customers" value={summary.response.available ? formatNumber(summary.response.unanswered.count) : "No data available"} helper="No valid human response" comparison={comparisonUnanswered(summary.response.unanswered.count, summary.comparison?.response.unanswered.count)} icon="alert" tone="red" href={responseDrilldownHref(activeStoreId, from, to, "unanswered")} /><MetricCard label="Customer Voice coverage" value={customerVoice ? formatPercentage(customerVoice.coverage.classifiedPercentage) : "No data available"} helper={customerVoice ? customerVoice.coverage.classifiedConversations.toLocaleString() + " / " + customerVoice.coverage.totalConversations.toLocaleString() + " classified" : "Current analysis data unavailable"} icon="voice" tone="purple" /></div></section>
          <section className="grid items-start gap-5 lg:grid-cols-[minmax(0,1.35fr)_minmax(270px,0.95fr)_minmax(250px,0.85fr)]"><BusinessPerformance summary={summary} /><CustomerVoicePanel data={customerVoice} loading={customerVoiceLoading} error={customerVoiceError} storeId={activeStoreId} from={from} to={to} /><SalesPerformance summary={summary} /></section>
          <section className="grid gap-5 lg:grid-cols-[minmax(0,1.2fr)_minmax(250px,0.95fr)_minmax(0,1.2fr)]"><section className={surfaceClass("h-full p-5 sm:p-6")} aria-labelledby="response-performance-title"><SectionHeading title="Response Performance" description="How quickly are customers receiving a valid human response?" headingId="response-performance-title" /><div className="mb-5 grid gap-3 sm:grid-cols-3"><Link href={responseDrilldownHref(activeStoreId, from, to, "within-24h")} className="rounded-xl bg-[var(--app-success-soft)] p-3 transition-colors hover:ring-2 hover:ring-[var(--app-accent)]/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--app-accent)]/40"><p className="text-[10px] text-[var(--app-text-secondary)]">Within 24h</p><p className="mt-1 text-xl font-bold tabular-nums text-[var(--app-success)]">{formatPercentage(summary.response.repliedWithin24Hours.percentage)}</p></Link><Link href={responseDrilldownHref(activeStoreId, from, to, "all")} className="rounded-xl bg-[var(--app-info-soft)] p-3 transition-colors hover:ring-2 hover:ring-[var(--app-info)]/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--app-info)]/40"><p className="text-[10px] text-[var(--app-text-secondary)]">Median first response</p><p className="mt-1 text-xl font-bold tabular-nums text-[var(--app-info)]">{formatDuration(summary.response.medianFirstResponseSeconds)}</p></Link><Link href={responseDrilldownHref(activeStoreId, from, to, "unanswered")} className="rounded-xl bg-[var(--app-warning-soft)] p-3 transition-colors hover:ring-2 hover:ring-[var(--app-accent)]/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--app-accent)]/40"><p className="text-[10px] text-[var(--app-text-secondary)]">Unanswered</p><p className="mt-1 text-xl font-bold tabular-nums text-[var(--app-warning)]">{formatNumber(summary.response.unanswered.count)}</p></Link></div><ResponseBars summary={summary} storeId={activeStoreId} from={from} to={to} /></section><ResponderList responders={responders} storeId={activeStoreId} /><ConversationExplorer conversations={visibleConversations} total={conversationTotal} loading={conversationLoading} activeStoreId={activeStoreId} searchText={searchText} setSearchText={setSearchText} responseFilter={responseFilter} setResponseFilter={setResponseFilter} responderFilter={responderFilter} setResponderFilter={setResponderFilter} salesFilter={salesFilter} setSalesFilter={setSalesFilter} responders={responders} customerVoiceTopic={customerVoiceTopic} selectCustomerVoiceTopic={selectCustomerVoiceTopic} onOpenConversation={(conversationId) => router.push("/chats?storeId=" + encodeURIComponent(activeStoreId) + "&conversationId=" + encodeURIComponent(conversationId))} /></section>
