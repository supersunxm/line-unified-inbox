...
import { Store360Shell } from "./store-360-shell";
...
export function Store360View() {
  ...
  return (
    <Store360Shell authUser={authUser} language={language} changeLanguage={setLanguage} logout={logout} isLoading={loading && !summary} apiError={error} onRetry={() => { void loadSummary(); void loadCustomerVoice(); void loadConversations(); }} style={biWorkspaceStyle}>
      <PageContainer variant="wide" className="bg-[var(--app-bg)]" style={biWorkspaceStyle}>
        {!stores.length && !loading ? <div className={surfaceClass("p-8 text-center text-sm text-[var(--app-text-secondary)]")}>No authorized stores available.</div> : loading && !summary ? <Store360Skeleton /> : summary ? <div className="space-y-5">
          <header className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between"><div className="min-w-0"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-[var(--app-accent)]">Store 360</p><h1 className="mt-1 text-[30px] font-bold leading-tight tracking-[-0.045em] text-[var(--app-text-primary)]">Store 360</h1><p className="mt-1 text-sm text-[var(--app-text-secondary)]">Turn conversations into business impact</p></div><div className="flex w-full flex-wrap items-center gap-2 xl:w-auto xl:flex-nowrap xl:justify-end xl:pr-48"><UnifiedPeriodPicker key={"store-360-period-picker-" + preset} dateFrom={from} dateTo={to} language={language} deferQuickRanges showOutsideQuickRanges={false} onApply={applyCustomRange} /><Store360ExportControl stores={stores} selectedStoreId={activeStoreId} startDate={from} endDate={to} language={language} /></div></header>
