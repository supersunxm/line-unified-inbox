...
export function CustomerVoicePanel({ data, loading, error, selectedTopic, onTopicSelect }: CustomerVoicePanelProps) {
  return (
    <section className={`${surfaceClass()} p-5 sm:p-6`} aria-labelledby="customer-voice-title">
      ...
      {loading ? <div className="grid gap-3 sm:grid-cols-3"><div className="h-28 animate-pulse rounded-xl bg-[var(--app-surface-subtle)]" /><div className="h-28 animate-pulse rounded-xl bg-[var(--app-surface-subtle)]" /><div className="h-28 animate-pulse rounded-xl bg-[var(--app-surface-subtle)]" /></div> : error ? <div className="rounded-xl border border-[var(--app-danger)]/30 bg-[var(--app-danger-soft)] p-4"><p className="text-sm font-semibold text-[var(--app-danger)]">Customer Voice is temporarily unavailable</p><p className="mt-1 text-xs text-[var(--app-text-secondary)]">{error}</p></div> : !data ? <p className="text-xs text-[var(--app-text-tertiary)]">Customer Voice analysis is not available yet.</p> : <>
