...
export function LanguageControl({ className = "" }: { className?: string }) {
  ...
  return (
    <div
      className={`inline-flex rounded-xl border border-[var(--app-border,#cbd5e1)] bg-[var(--app-surface,#fff)] p-1 shadow-sm ${className}`}
      role="group"
      ...
      {(Object.keys(languageNames) as AppLanguage[]).map((item) => (
        <button
          ...
          className={`min-h-8 rounded-lg px-2.5 text-xs font-semibold transition-colors ${
            language === item
              ? "bg-[var(--app-accent,#111827)] text-white"
              : "text-[var(--app-text-secondary,#475569)] hover:bg-[var(--app-surface-subtle,#f1f5f9)]"
          }`}
        >
          {languageNames[item]}
        </button>
