    ...
    <>
      ...
      <div className="space-y-4 px-4 py-4 pb-8">
        ...
        <MobileSection title="LINE Official Accounts" description={`${row.accounts.length} บัญชี`}>
          {row.accounts.length === 0 ? <MobileEmptyState title="ยังไม่มี LINE OA" description="ร้านนี้ยังไม่มีบัญชี LINE OA ที่เชื่อมกับระบบ" /> : <div className="space-y-2.5">{row.accounts.map((account) => <MobileListCard key={account.id} title={account.name} subtitle={account.basicId ?? account.store.externalStoreId ?? "—"} trailing={<StatusPill status={account.connectionStatus} healthy={account.credentialsHealthy} />}><div className="grid grid-cols-2 gap-2"><MiniStat label="บทสนทนา" value={account.conversationCount} /><MiniStat label="ข้อความวันนี้" value={account.messagesReceivedToday} /></div>{account.lastConnectionError && <p className="mt-2 rounded-lg bg-rose-500/10 px-2.5 py-2 text-[10px] leading-4 text-rose-600 dark:text-rose-400">{account.lastConnectionError}</p>}<div className="mt-3 grid grid-cols-2 gap-2"><button type="button" disabled={actionLoading === account.id} onClick={() => onTest(account)} className="min-h-10 rounded-xl border border-[var(--app-border)] text-[10px] font-bold disabled:opacity-40">{actionLoading === account.id ? "กำลังตรวจ..." : "Test Connection"}</button>{account.resolvedLineOaManagerUrl ? <a href={account.resolvedLineOaManagerUrl} target="_blank" rel="noreferrer" className="flex min-h-10 items-center justify-center rounded-xl bg-[var(--app-accent)] px-2 text-center text-[10px] font-bold text-white">เปิด LINE OA</a> : <button type="button" disabled className="min-h-10 rounded-xl bg-[var(--app-surface-subtle)] text-[10px] text-[var(--app-text-tertiary)]">ไม่มี Manager URL</button>}</div>{user.role === "ADMIN" && <button type="button" disabled={actionLoading === account.id} onClick={() => onSetActive(account, !account.isActive)} className={`mt-2 min-h-10 w-full rounded-xl text-[10px] font-bold ${account.isActive ? "border border-amber-500/30 text-amber-600" : "bg-emerald-600 text-white"}`}>{account.isActive ? "ปิดการใช้งานบัญชี" : "เปิดการใช้งานบัญชี"}</button>}</MobileListCard>)}</div>}
        </MobileSection>
