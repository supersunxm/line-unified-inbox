...
type IconName = "home" | "dashboard" | "chat" | "main-oa" | "followers" | "traffic" | "coupon" | "store" | "store-360" | "purchase" | "friend" | "broadcast" | "approval" | "health" | "tiktok" | "rich-menu" | "auto-response" | "greeting" | "review" | "download" | "profile" | "settings" | "logout";
...
function SidebarIcon({ name, className = "" }: { name: IconName; className?: string }) {
  ...
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" className={`h-5 w-5 ${className}`}>
      ...
      {name === "store-360" && <><rect {...common} x="3" y="4" width="18" height="16" rx="2"/><path {...common} d="M7 16v-3m4 3V9m4 7v-5m4 5v-8"/></>}
      ...
      {name === "auto-response" && <><path {...common} d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2 2z"/><path {...common} d="M13 8l-3 4h4l-2 4"/></>}
      {name === "greeting" && <><path {...common} d="M18 8a3 3 0 0 0-3-3 3 3 0 0 0-3 3v8a2 2 0 0 1-2 2 2 2 0 0 1-2-2V8"/><path {...common} d="M10 10V5a2 2 0 0 1 4 0v5"/><path {...common} d="M14 10V6a2 2 0 0 1 4 0v4"/><path {...common} d="M6 13V9a2 2 0 0 1 4 0v4"/></>}
      {name === "review" && <><path {...common} d="m12 2 3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2Z"/></>}
      {name === "download" && <><path {...common} d="M12 3v11"/><path {...common} d="m8 10 4 4 4-4"/><path {...common} d="M5 19h14"/></>}
...
function labels(language: Language) {
  if (language === "th") return { workspace: "พื้นที่ทำงาน", mainOa: "Main OA", operations: "เครื่องมือ", home: "หน้าหลัก", dashboard: "แดชบอร์ด", replySla: "อัตราตอบกลับ 24 ชม.", chats: "แชทร้านค้า", followers: "ข้อมูลผู้ติดตาม", traffic: "Message Traffic", coupons: "คูปอง", storeDirectory: "ค้นหาสาขา", stores: "จัดการร้านค้า", store360: "Store 360", purchase: "ข้อมูลการซื้อ", friendLinks: "ลิงก์เพิ่มเพื่อน", mass: "ส่งข้อความ", approval: "อนุมัติ BM", richMenus: "จัดการ Rich Menu", autoResponses: "ข้อความตอบกลับอัตโนมัติ", greetingMessages: "ข้อความต้อนรับ", googleReviewKpi: "Google Review KPI", downloadCenter: "Download Center", profile: "โปรไฟล์", settings: "ตั้งค่า", logout: "ออกจากระบบ", language: "ภาษา", appearance: "รูปแบบการแสดงผล", role: "สิทธิ์การใช้งาน", environment: "สภาพแวดล้อม", standard: "Production", pilot: "Pilot", collapse: "ย่อแถบเมนู", expand: "ขยายแถบเมนู" };
  if (language === "zh") return { workspace: "工作区", mainOa: "Main OA", operations: "工具", home: "主页", dashboard: "仪表盘", replySla: "24小时回复率", chats: "门店聊天", followers: "关注者洞察", traffic: "消息流量", coupons: "优惠券", storeDirectory: "门店目录", stores: "门店管理", store360: "Store 360", purchase: "购买洞察", friendLinks: "加好友来源链接", mass: "群发消息", approval: "BM 审批", richMenus: "Rich Menu 管理", autoResponses: "自动回复", greetingMessages: "欢迎消息", googleReviewKpi: "Google Review KPI", downloadCenter: "下载中心", profile: "个人资料", settings: "设置", logout: "退出", language: "语言", appearance: "外观", role: "角色", environment: "环境", standard: "Production", pilot: "Pilot", collapse: "收起侧栏", expand: "展开侧栏" };
  return { workspace: "Workspace", mainOa: "Main OA", operations: "Tools", home: "Main", dashboard: "Dashboard", replySla: "24H Response Rate", chats: "Store Chats", followers: "Follower Insights", traffic: "Message Traffic", coupons: "Coupons", storeDirectory: "Store Directory", stores: "Store Management", store360: "Store 360", purchase: "Purchase Intelligence", friendLinks: "Friend Source Links", mass: "Mass Message", approval: "BM Approval", health: "LINE Chat Health", richMenus: "Rich Menu Manager", autoResponses: "Auto-response", greetingMessages: "Greeting Messages", googleReviewKpi: "Google Review KPI", downloadCenter: "Download Center", profile: "Profile", settings: "Settings", logout: "Logout", language: "Language", appearance: "Appearance", role: "Role", environment: "Environment", standard: "Production", pilot: "Pilot", collapse: "Collapse sidebar", expand: "Expand sidebar" };
}
...
export function AppSidebar({ authUser, changeLanguage, currentSection, language, logout, pilotMode, text }: SidebarProps) {
  ...
  const primaryItems: NavItem[] = [
    ...
    { href: "/replymessage", label: t.replySla, icon: "traffic", section: "dashboard", active: (path) => path.startsWith("/replymessage") },
    { href: "/chats", label: t.chats, icon: "chat", section: "chats", active: (path) => path.startsWith("/chats") },
    { href: "/follower-insights", label: t.followers, icon: "followers", section: "follower-insights" },
    { href: "/store-360", label: t.store360, icon: "store-360", section: "store-360", active: (path) => path.startsWith("/store-360") },
  ...
  const toolItems: NavItem[] = [
    { href: "/download-center", label: t.downloadCenter, icon: "download", section: "follower-insights", active: (path) => path.startsWith("/download-center") },
    { href: "/coupons", label: t.coupons, icon: "coupon", section: "coupons" },
    { href: "/stores", label: t.storeDirectory, icon: "store", active: (path) => path === "/stores" },
    { href: "/storemanagement", label: t.stores, icon: "store", section: "stores", active: (path) => path.startsWith("/storemanagement") || path.startsWith("/admin/stores") },
    { href: "/google-review-kpi", label: t.googleReviewKpi, icon: "review", section: "google-review-kpi" },
