...
function labels(language: Language) {
  if (language === "th") {
    return {
      ...
      operations: "เครื่องมือ",
      dashboard: "แดชบอร์ด",
      chats: "แชทร้านค้า",
      followers: "ข้อมูลผู้ติดตาม",
      traffic: "Message Traffic",
      coupons: "คูปอง",
      stores: "จัดการร้านค้า",
      purchase: "ข้อมูลการซื้อ",
      classification: "ข้อมูลการจำแนก",
      friendLinks: "ลิงก์เพิ่มเพื่อน",
      mass: "ส่งข้อความ",
      approval: "อนุมัติ BM",
      profile: "โปรไฟล์",
      settings: "ตั้งค่า",
  ...
  if (language === "zh") {
    return {
      workspace: "工作区", operations: "工具", dashboard: "仪表盘", chats: "门店聊天", followers: "关注者洞察", traffic: "消息流量", coupons: "优惠券", stores: "门店管理", purchase: "购买洞察", classification: "分类洞察", friendLinks: "加好友链接", mass: "群发消息", approval: "BM 审批", profile: "个人资料", settings: "设置", logout: "退出", language: "语言", appearance: "外观", role: "角色", environment: "环境", standard: "Production", pilot: "Pilot", collapse: "收起侧栏", expand: "展开侧栏",
    };
  }
  return {
    workspace: "Workspace", operations: "Tools", dashboard: "Dashboard", chats: "Store Chats", followers: "Follower Insights", traffic: "Message Traffic", coupons: "Coupons", stores: "Store Management", purchase: "Purchase Intelligence", classification: "Classification Insights", friendLinks: "Friend Source Links", mass: "Mass Message", approval: "BM Approval", profile: "Profile", settings: "Settings", logout: "Logout", language: "Language", appearance: "Appearance", role: "Role", environment: "Environment", standard: "Production", pilot: "Pilot", collapse: "Collapse sidebar", expand: "Expand sidebar",
  };
}
...
export function AppSidebar({ authUser, changeLanguage, currentSection, language, logout, pilotMode, text }: SidebarProps) {
  ...
  ], [t.chats, t.dashboard, t.followers, t.traffic]);
  ...
  const toolItems = useMemo<NavItem[]>(() => [
    ...
    { href: "/tiktok", label: "TikTok", icon: "♪", external: false, adminOnly: true },
  ], [t.approval, t.classification, t.coupons, t.friendLinks, t.mass, t.purchase, t.stores]);
  ...
  return (
    <aside className={`app-desktop-sidebar fixed inset-y-0 left-0 z-40 hidden border-r border-[var(--app-border)] bg-[var(--app-surface)] transition-[width] duration-200 md:flex md:flex-col ${collapsed ? "w-[4.5rem]" : "w-64"}`}>
        ...
        <Link href="/dashboard" className={`${focusRing} flex min-w-0 items-center gap-2 rounded-lg`} title={collapsed ? (text.appName || "OPPO LINE OA Monitor") : undefined}>
          ...
          {!collapsed && (
            <div className="min-w-0">
              <div className="truncate text-sm font-bold text-[var(--app-text-primary)]">{text.appName || "OPPO LINE OA Monitor"}</div>
              <div className="truncate text-[10px] text-[var(--app-text-tertiary)]">Retail Operations</div>
            </div>
          )}
        </Link>
        {!collapsed && (
          <button type="button" onClick={() => setCollapsed(true)} aria-label={t.collapse} title={t.collapse} className={`${focusRing} flex h-8 w-8 items-center justify-center rounded-lg text-[var(--app-text-tertiary)] hover:bg-[var(--app-surface-hover)] hover:text-[var(--app-text-primary)]`}>‹</button>
        )}
      </div>

      {collapsed && (
        <button type="button" onClick={() => setCollapsed(false)} aria-label={t.expand} title={t.expand} className={`${focusRing} mx-auto mt-2 flex h-9 w-9 items-center justify-center rounded-xl text-[var(--app-text-tertiary)] hover:bg-[var(--app-surface-hover)] hover:text-[var(--app-text-primary)]`}>›</button>
      )}
      ...
      <div className="relative shrink-0 border-t border-[var(--app-border-subtle)] p-2">
            ...
            {accountPanel === "profile" ? (
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="rounded-xl bg-[var(--app-surface-subtle)] p-2.5">
                    <div className="text-[10px] text-[var(--app-text-tertiary)]">{t.role}</div>
                    <div className="mt-0.5 text-xs font-semibold text-[var(--app-text-primary)]">{authUser.role}</div>
                  </div>
                  <div className="rounded-xl bg-[var(--app-surface-subtle)] p-2.5">
                    <div className="text-[10px] text-[var(--app-text-tertiary)]">{t.environment}</div>
                    <div className="mt-0.5 text-xs font-semibold text-[var(--app-text-primary)]">{pilotMode ? t.pilot : t.standard}</div>
                  </div>
                </div>
              </div>
            ) : (
              <div className="space-y-4">
                <label className="block text-xs font-medium text-[var(--app-text-secondary)]">
                  <span className="mb-1.5 block">{t.language}</span>
                  <select value={language} onChange={(event) => changeLanguage(event.target.value as Language)} className={`${focusRing} h-10 w-full rounded-xl border border-[var(--app-border)] bg-[var(--app-surface-subtle)] px-3 text-sm text-[var(--app-text-primary)]`}>
                ...
                <div>
                  <div className="mb-1.5 text-xs font-medium text-[var(--app-text-secondary)]">{t.appearance}</div>
                  <ThemeControl />
                </div>
              </div>
            )}
        ...
        <button type="button" onClick={() => { if (collapsed) setCollapsed(false); setAccountPanel((current) => current === "profile" ? null : "profile"); }} className={itemClass(accountPanel === "profile")} title={collapsed ? t.profile : undefined}>
          <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[var(--app-accent)] text-[10px] font-bold text-white">{authUser.displayName.charAt(0).toUpperCase()}</span>
          {!collapsed && <span className="min-w-0 flex-1 truncate text-left">{t.profile}</span>}
          {!collapsed && <span className="text-[11px] text-[var(--app-text-tertiary)]">{authUser.role}</span>}
        </button>
        <button type="button" onClick={() => { if (collapsed) setCollapsed(false); setAccountPanel((current) => current === "settings" ? null : "settings"); }} className={itemClass(accountPanel === "settings")} title={collapsed ? t.settings : undefined}>
          <span className="flex h-7 w-7 shrink-0 items-center justify-center text-[17px]" aria-hidden="true">⚙</span>
          {!collapsed && <span className="min-w-0 flex-1 truncate text-left">{t.settings}</span>}
        </button>
        <button type="button" onClick={() => void logout()} className={`${itemClass(false)} text-[var(--app-danger)] hover:bg-[var(--app-danger-soft)] hover:text-[var(--app-danger)]`} title={collapsed ? t.logout : undefined}>
          <span className="flex h-7 w-7 shrink-0 items-center justify-center text-[17px]" aria-hidden="true">↪</span>
          {!collapsed && <span className="min-w-0 flex-1 truncate text-left">{t.logout}</span>}
        </button>
      </div>
    </aside>
  );
