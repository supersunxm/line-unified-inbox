...
async function main(): Promise<void> {
  ...
  const conversationId = process.env.LINE_CHAT_COMPOSER_DIAGNOSTIC_CONVERSATION_ID?.trim() || "";
  ...
  try {
    ...
    const result = await relay.inspectComposerControls({ conversationId });
    console.log(JSON.stringify({
      event: "line_chat_manager_composer_control_diagnostic",
      conversationId,
      result,
    }));
  } finally {
...
void main().catch((error) => {
  console.error(JSON.stringify({
    event: "line_chat_manager_composer_control_diagnostic_fatal",
