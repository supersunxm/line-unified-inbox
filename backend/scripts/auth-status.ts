...
import { emailFromAddress } from "../src/email/email.config";
...
async function main() {
  try {
    ...
    const emailProviderConfigured = mode === "console" ? process.env.NODE_ENV !== "production" : mode === "resend" && Boolean(process.env.RESEND_API_KEY?.trim() && emailFromAddress());
