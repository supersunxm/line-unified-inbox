...
if [ "${LINE_CHAT_RECOVER_CONFIRMED_OUTBOUNDS_ENABLED:-false}" = "true" ]; then
  ...
  RECOVERY_MESSAGES="${LINE_CHAT_RECOVER_CONFIRMED_OUTBOUNDS_MESSAGES:-}"
  ...
  if [ "$RECOVERY_APPLY" = "true" ]; then
    npx tsx scripts/recover-confirmed-failed-outbounds.ts --store="$RECOVERY_STORE" --customers="$RECOVERY_CUSTOMERS" --conversation-ids="$RECOVERY_CONVERSATIONS" --message-ids="$RECOVERY_MESSAGES" --from="$RECOVERY_FROM" --to="$RECOVERY_TO" --apply
  else
    npx tsx scripts/recover-confirmed-failed-outbounds.ts --store="$RECOVERY_STORE" --customers="$RECOVERY_CUSTOMERS" --conversation-ids="$RECOVERY_CONVERSATIONS" --message-ids="$RECOVERY_MESSAGES" --from="$RECOVERY_FROM" --to="$RECOVERY_TO"
  fi
  RECOVERY_EXIT_CODE=$?
  set -e
...
if [ "${LINE_CHAT_MESSAGE_API_INSPECT_ENABLED:-false}" = "true" ]; then
  ...
  set +e
  npx tsx scripts/inspect-manager-message-api.ts
  MESSAGE_API_INSPECT_EXIT_CODE=$?
  set -e
  echo "{\"event\":\"line_chat_message_api_inspection_bootstrap_finished\",\"exitCode\":${MESSAGE_API_INSPECT_EXIT_CODE}}"
fi
...
if [ "${LINE_CHAT_TIMELINE_DIAGNOSTIC_ENABLED:-false}" = "true" ]; then
  ...
  set +e
  npx tsx scripts/inspect-manager-chat-timeline.ts
  TIMELINE_DIAGNOSTIC_EXIT_CODE=$?
  set -e
  echo "{\"event\":\"line_chat_manager_timeline_diagnostic_bootstrap_finished\",\"exitCode\":${TIMELINE_DIAGNOSTIC_EXIT_CODE}}"
fi

if [ "${LINE_CHAT_COMPOSER_DIAGNOSTIC_ENABLED:-false}" = "true" ]; then
  echo "{\"event\":\"line_chat_manager_composer_control_diagnostic_bootstrap_started\"}"
  set +e
  npx tsx scripts/inspect-manager-composer-controls.ts
  COMPOSER_DIAGNOSTIC_EXIT_CODE=$?
  set -e
  echo "{\"event\":\"line_chat_manager_composer_control_diagnostic_bootstrap_finished\",\"exitCode\":${COMPOSER_DIAGNOSTIC_EXIT_CODE}}"
fi
