...
-- Safety: seed only when the canonical active Store is mapped with
-- Store.code = 28375 and StoreMaster.externalStoreId = 28375.
-- Clean/CI databases without production store data must migrate successfully
-- without creating pilot rules.
DO $$
DECLARE
  v_store_id TEXT;
BEGIN
  ...
  IF v_store_id IS NULL THEN
    RAISE NOTICE 'Robinson Chonburi store 28375 is not present; skipping shadow auto-response rule seed.';
  ELSE
