...
describe("findSoleManagerTextarea", () => {
  it("returns the sole non-search textarea only when LINE Manager exposes it as naturally editable", async () => {
    const result = await findSoleManagerTextarea(fakeFrame({ editable: true }));
    expect(result).not.toBeNull();
  });

  it("does not force-enable a disabled/readonly textarea", async () => {
