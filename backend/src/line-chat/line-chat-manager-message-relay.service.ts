...
export class LineChatManagerMessageRelayService {
  ...
  /**
   * Relay text through LINE OA Manager only for the Phase-1 Chonburi store.
   * Non-pilot conversations return handled=false so normal Messaging API logic
   * remains untouched. Once a conversation resolves to store 28375, failures
   * fail closed: callers must never fall back to Push API for that message.
   */
  public async relayText(input: {
    ...
    const dedupeKey = `${conversationId}:${input.idempotencyKey}`;
  ...
  private async executePilotRelay(
    ...
    const sendOperation = await this.coordinator.withProfileOperation(
      // MANUAL_DIAGNOSTIC is intentionally reused for this tightly-scoped
      // Phase-1 browser operation so no production Prisma enum migration is
      // required before the Chonburi proof-of-concept is validated.
  ...
  private assertPilotConfiguration(conversation: PilotConversation): void {
    ...
    if (
      ...
      throw new ServiceUnavailableException("การตั้งค่า LINE OA Manager ของร้านชลบุรีไม่พร้อมใช้งาน จึงยกเลิกการส่งเพื่อป้องกันการใช้ Push quota");
    }
  }
  ...
  private async findComposer(page: Page): Promise<Locator | null> {
    ...
    for (const selector of COMPOSER_SELECTORS) {
      ...
      for (let i = 0; i < count; i += 1) {
        ...
        // The message composer is in the lower portion of the chat pane. This
        // prevents accidentally typing into the top search field.
