...
@Module({ imports: [EmailModule], controllers: [AuthController, RegistrationController, AdminRegistrationController, AdminAuditLogController], providers: [PasswordService, AuthService, AuthRateLimitService, AuditLogService, SetupService, DevAdminService, PilotAdminBootstrapService, StoreAccessService, { provide: OTP_CODE_GENERATOR, useValue: () => randomInt(0, 1_000_000).toString().padStart(6, "0") }, { provide: SMS_PROVIDER, useFactory: () => process.env.NODE_ENV === "production" || process.env.SMS_PROVIDER === "smsmkt" ? new SmsMktProvider() : new DevelopmentSmsProvider() }, OtpChallengeService, RegistrationService, MobileAuthService, { provide: APP_GUARD, useClass: AuthGuard }], exports: [PasswordService, AuthService, StoreAccessService] })
