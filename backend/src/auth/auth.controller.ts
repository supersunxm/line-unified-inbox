...
export class AuthController {
  ...
  @Public() @Post("mobile/refresh") mobileRefresh(@Body() dto: MobileRefreshDto) { return this.auth.refreshMobileSession(dto.refreshToken); }
  ...
  @Public() @Post("mobile/logout") async mobileLogout(@Req() request: Request, @Body() dto: MobileLogoutDto) {
    ...
    await this.auth.logoutMobile(token, dto?.refreshToken);
    return { success: true };
  }
}
