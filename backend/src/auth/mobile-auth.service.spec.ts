...
void test("active BM creates a MOBILE session after valid OTP verification", async () => {
  ...
  const service = new MobileAuthService(prisma, otp, auth);
...
void test("pending, rejected, and suspended users cannot complete mobile login", async () => {
  for (const user of [
    ...
    const service = new MobileAuthService(prisma, { verify: async () => undefined } as any, { createSession: async () => { throw new Error("must not create session"); } } as any);
...
void test("wrong or expired OTP and duplicate verification do not create additional sessions", async () => {
  ...
  const service = new MobileAuthService(prisma, otp, { createSession: async () => ({ token: `token-${++sessionCount}`, expiresAt: new Date() }) } as any);
...
void test("/auth/me returns the authenticated mobile profile and mobile logout deletes only MOBILE sessions", async () => {
  ...
  let where: unknown;
  const auth = new AuthService({ session: { deleteMany: async ({ where: input }: { where: unknown }) => { where = input; return { count: 1 }; } } } as any, {} as any);
