...
export class MessageTrafficController {
  ...
  private async resolveAllowedStoreIds(req: AuthRequest | undefined, allowedStoreIdsRaw?: string) {
    ...
    if (requestedStoreIds.some((storeId) => !activeStoreIdSet.has(storeId))) throw new ForbiddenException("Requested store is not available");
  ...
  async getMessageTraffic(
    ...
    const safePeriod: MessageTrafficPeriod = period === "today" || period === "7d" || period === "30d" ? period : "30d";
    const from = this.parseDate(fromRaw, "from");
    const to = this.parseDate(toRaw, "to");
    if ((from && !to) || (!from && to)) throw new BadRequestException("from and to must be provided together");
