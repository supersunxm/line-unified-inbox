...
export class LineProfileService {
  ...
  async refresh(customerId: string, lineOfficialAccountId: string, force = false) {
    ...
    try {
      ...
      return this.prisma.customer.update({ where: { id: customerId }, data: { displayName: profile.displayName?.slice(0, 200) || customer.displayName, pictureUrl: profile.pictureUrl?.slice(0, 1000), statusMessage: profile.statusMessage?.slice(0, 500), preferredLanguage: profile.language?.slice(0, 20), profileFetchedAt: new Date(), profileFetchStatus: "SUCCESS", profileFetchError: null } });
    } catch (error) {
