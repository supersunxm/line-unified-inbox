    ...
    const conversation = await this.prisma.conversation.findUnique({
      ...
      select: {
        ...
        messages: {
          ...
          orderBy: [{ sentAt: "desc" }, { id: "desc" }],
          take: query.limit + 1,
          select: { id: true, direction: true, deliveryStatus: true, messageType: true, originalText: true, fileName: true, rawPayload: true, sentAt: true, senderUserId: true, senderDisplayName: true, sender: { select: { id: true, displayName: true } }, media: { select: { processingStatus: true, mimeType: true, fileSize: true } } },
        },
        _count: { select: { messages: { where: ownerTrackingInboundFilter() }, pushNotifications: { where: { userId: user.id, readAt: null } } } },
      },
    });
