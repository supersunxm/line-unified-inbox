...
- **Client-Side Modification Discovery**: Inspection of TokCounter's client bundle (`_next/static/chunks/4372h2xpjy22i.js`, module `10514`) revealed that TokCounter's web frontend programmatically applies a synthetic offset to like counts: `if (likes >= 10050 && likes <= 1049000) likes += 50`. Thus, raw DOM extraction for likes reflects `11,035` while the actual TikTok upstream returned by the underlying JSON payload is `10,985`. The scraper records DOM metrics at EXACT integer precision and preserves background network observations without switching to undocumented API calls.
- **Ephemeral Browser Context**: Standard headless Playwright Chromium operates cleanly with zero persistent profile dependencies, zero login, and zero CAPTCHAs. Browser instances and pages can be sequentially navigated across accounts without recreating browser processes.
- **Zero Production Mutation**: Per POC rules, this task produces only an isolated test script (`backend/scripts/tiktok-public/test-tokcounter-store.mjs`); no database migrations, schema edits, cron triggers, or full sheet scraping were performed.
...
- **Public query isolation**: `getStoreOwnerAnalytics()` calls a dedicated account-level snapshot method that selects only the required `TikTokAccount` and `StoreMaster` fields, with no `TikTokVideo` relation. The internal `getTikTokAccountById()` lookup remains unchanged for HQ consumers.
...
- **Safe response contract**: The store-owner DTO exposes only customer-facing profile counts, timestamps, selected store display data, and daily account-level metrics. Encrypted access/refresh tokens, internal account/store IDs, account names, video records, and staff controls are excluded.
- **Binding-state behavior**: Only a confirmed binding can return analytics. Pending requests return a neutral selected-store review state; unconfirmed sessions return to the association flow; no analytics data is fabricated when daily snapshots are unavailable.
...
# Conversation owner and Team Operations Summary boundary (2026-08-29)

- Ownership is a nullable relation from `Conversation` to `User`, not a derived “last responder.” Assignment is allowed only for active users authorized for the conversation store (active membership, or an active ADMIN/all-store account); bot, Main OA, stale, and deactivated owners serialize as null. The first successful human outbound assigns atomically with the persisted send, guarded by `ownerUserId: null` so later staff replies do not steal ownership.
...
# Mobile product picker boundary (2026-08-30)

- Keep the existing `/mobile/products` API and ProductSelectorItem payload as the catalog source. The Android picker requests the existing maximum (`limit=50`) once per open and applies search/category/series filtering locally; no catalog records, endpoint semantics, or backend migration change.
- Classify products from persisted category, series name, and product name with specific device-family checks first, then Smartphone model/series patterns, and place the remaining catalog in IoT. Smartphone secondary filters are Find, Reno, and safely matched A Series; unknown Smartphone series remain visible under All.
- Reset picker search/category/series on open/close and reset secondary series whenever the primary category changes. Product rows use a fully tappable compact Material row with a pale-green selected state/check icon, while the existing variant, quantity, immediate-save, rollback, and confirmation flows remain authoritative.
# Realtime pilot LINE chat resolver boundary (2026-09-01)

- Phase 1 resolves missing LINE OA Manager chat IDs only on demand inside the dedicated nickname worker for future ONLINE/PURCHASED saves at pilot Store 28375. Normal backend HTTP processes never own Playwright, and mapped conversations retain the existing queue/worker path.
- The realtime resolver is intentionally distinct from historical enumeration: it reads at most five recent v2 chat-list pages (125 records), requires exactly one normalized exact-name plus ±60-second timestamp match, accepts only USER `chatId` values matching `^U[0-9a-f]{32}$`, and never consults response `userId` or `Customer.lineUserId`.
- A candidate must be unused by another conversation for the same OA. Persistence runs in a serializable transaction and uses an atomic target/OA/null guard; only `Conversation.lineChatUserId` changes, and a race proceeds only when a re-read equals the same candidate. Resolution failures stay inside the nickname job and cannot roll back BM sales data.
- Phase 2 will use manual historical mapping and nickname correction. Historical bulk auto-mapping, full-account scans, external-message correlation, and historical nickname backfill are paused and unchanged by this work.
# Realtime resolver diagnostic privacy boundary (2026-09-01)
...
- Use a dedicated `store-insights` backend domain with server-side aggregation over the existing Store, LINE OA, Conversation, Message, sales/product, follower snapshot, User, and ActivityHistory records.
- Treat a valid human response as an OUTBOUND message with a real `senderUserId` whose effective display name is not `Auto Reply Bot`; exclude SYSTEM and automated messages. If an ambiguous outbound row exists in the selected store/period, response-rate, response-time, and unanswered metrics are marked unavailable rather than guessed.
- Scope one response case per conversation to the first inbound activity in the selected Bangkok-calendar range and pair it with the first later valid human outbound response observed through the bounded report-end-plus-24-hour response window. This makes SLA, median first response, unanswered, and responder metrics share one definition while allowing boundary-crossing replies.
- Return only aggregates and bounded conversation explorer rows from Store Insights; raw message bodies are never sent to the browser for analytics.
- Keep Phase 2 AI conversation analysis out of core tables. If needed later, add a separate `ConversationAnalytics`/`conversation_analytics` domain keyed by conversation ID with model/version and processed-at provenance.
- Reuse the existing `/chats?conversationId=` navigation for conversation drill-through; Store 360 does not introduce a second chat viewer.
- No database migration is planned for Phase 1 because existing indexes on `Conversation.storeId`, `Message.conversationId/sentAt`, and `LineOaFollowerSnapshot.lineOaId/snapshotDate` cover the initial bounded queries. Revisit composite indexes after production query-cost observation.

## Store 360 Phase 1 production-readiness semantics (2026-09-10)

- The report range is a Bangkok-calendar range represented as an inclusive `from`/`to` date pair with an exclusive UTC database end boundary. Inbound selection stays inside that range; response lookup extends only through `period.end + 24 hours` so a reply just after the reporting boundary can satisfy the inbound case's SLA. No response beyond that bounded horizon affects Store 360.
- The measurement unit is one response case per conversation: the earliest selected-period inbound starts the case and the earliest later qualifying human outbound ends it. Additional inbound messages before that reply remain part of the same conversation-level case, so SLA, median first response, and unanswered use the same unit.
- A qualifying human outbound is an `OUTBOUND` message with non-null `senderUserId` and an effective sender name other than `Auto Reply Bot`. Current text, image, and mobile video sends persist this identity. Auto-response/bot, `SYSTEM`, and unattributed legacy outbound rows do not satisfy a case; any ambiguous outbound in the bounded snapshot fails closed for response percentages and time metrics.
- `Unanswered` means the case has no qualifying human outbound observed through the bounded report-end-plus-24-hour response horizon. A human reply observed after 24 hours is therefore replied-but-late: it is excluded from the 24-hour bucket while remaining available for first-response duration/median.
...
# Store 360 Phase 2A: Customer Voice (2026-09-10)

- Use existing `ConversationTopic` → `Topic` rows as a read-only signal and map them into one versioned canonical Customer Voice taxonomy. Preserve source provenance (`EXISTING_TOPIC`, enriched, AI, or unclassified) instead of creating duplicate topics or writing inferred topics into the shared taxonomy.
- Persist Customer Voice in a separate `ConversationAnalytics` model with a compound conversation/version uniqueness constraint. Reprocessing the same conversation/version upserts one row, while a later inbound message can be reanalyzed without duplicate analytics rows.
- Analyze only real inbound customer text at conversation level. Multiple inbound bubbles in one conversation produce one analysis; no classification runs during Store 360 page rendering, and API responses expose labels/counts/coverage rather than raw message bodies.
- Use the deterministic rules engine and existing product catalog/alias matcher as the provider-neutral fallback because no LLM provider SDK, credentials, or approved paid dependency exists in the repository. Do not fabricate products or spend on AI calls in this phase.
- Enforce the existing server-side Store 360 boundary: authorized store, active STORE LINE OA, non-QA conversations, and no HEAD_OFFICE scope. Keep current/previous date semantics in Bangkok calendar days with bounded 90-day requests and zero-safe comparison trends.
- Expose Customer Voice through a dedicated authenticated endpoint and place its UI after the Store 360 KPI/response sections. Topic selection reuses the existing Conversation Explorer rather than introducing a second conversation viewer.
...
## 2026-09-12: Store/StoreMaster/LINE OA identity integrity

- Treat Store code and LINE Basic ID as independent identity authorities during reconciliation. If each uniquely resolves a different active StoreMaster, record a sanitized `IDENTITY_CONFLICT` and make no OA, Store, or conversation rebind based on that account. Account-name matching is fallback evidence only and never overrides a Basic-ID conflict.
- Enforce at most one active, non-archived STORE OA per Store with a PostgreSQL partial unique index. Archived historical OAs remain allowed on the same Store. The migration fails closed when conflicting active rows exist and is deployment-idempotent because the emergency production index was installed before the application deployment.
- When a StoreMaster is explicitly selected, fail with `STORE_MASTER_IDENTITY_MISMATCH` unless its external Store ID matches the selected Store code, its populated LINE ID matches the submitted Basic ID, and no other active STORE OA occupies the Store. Keep global active Basic ID, Channel ID, and Destination ID checks as separate `LINE_ACCOUNT_DUPLICATE` protections.
- A confirmed OA rebind and all of that OA's `Conversation.storeId` updates are one transaction. Repair scripts revalidate the reviewed source and target identities immediately before writing and never mutate credentials, webhook keys, LINE identifiers, messages, or unrelated history.

## 2026-09-12: StoreMaster Sheet row numbers are provenance only

- Resolve every StoreMaster row with a real Store ID by its canonical `source + externalStoreId`, never by Sheet position. Always refresh `sourceRowNumber` on the resolved identity so reorder history remains observable without changing identity.
...
- Authenticated production-mode verification is the release gate: the rebuilt frontend showed zero `/webhook-info` requests on `/chats` over 10m23s, while Stores management retained exactly one initial 145-account diagnostic wave and no silent recurrence. This validates the guard without changing the remaining shared polling datasets.
- Deploy only the frontend service for this frontend-only patch. Railway deployment `bdc7c238-420e-411b-abeb-068b9b430ab1` succeeded for commit `4752172`; the backend deployment was skipped by its existing `backend/**` watch pattern.

## 2026-09-15: P1 route-scoped shared dataset polling

- Keep the P1 change limited to `ApplicationWorkspace` polling. Chats receives full stores/OAs/products/topics/BM metadata on entry and polls only conversations plus BM summary every 12 seconds. Store Management receives stores/OAs and keeps a 60-second passive refresh. Other workspace sections do not mount the shared supporting-data loader; their existing route-owned loaders remain authoritative.
- Preserve Store Management webhook behavior as an explicit operation. Initial/manual Store Management loads and existing mutation refreshes continue to opt into webhook diagnostics; passive polls never do. Do not change webhook endpoints, payloads, cancellation behavior, backend contracts, or UI.
- Do not optimize the remaining shared datasets in this change. The route-scoped split is the smallest safe reduction supported by the measured request patterns and existing consumers; cancellation and further dataset-specific changes remain separate follow-ups.
- Treat the local authenticated production-build measurements as the release gate for this checkpoint. The corrected `/chats` window reduced 82 to 29 requests over two minutes and eliminated webhook diagnostics; Store Management retained one intentional local diagnostic wave and added only one 60-second stores/OA refresh. Keep the branch uncommitted and undeployed pending review.

## 2026-09-15: 6-digit PIN authentication

- Attach PIN credentials to the existing `User` and mobile authentication flow. Password login remains the enrollment and recovery path; successful PIN login issues the same mobile access/refresh session pair and uses the existing authorization/session restore path.
- Store only scrypt PIN hashes with additive nullable/status/lock metadata. Existing users migrate to `pinHash = null`, `pinEnabled = false`, zero failed attempts, and no changes to `passwordHash`.
- Enforce exactly six numeric digits, backend per-account lockout after five failures for 15 minutes, and the existing IP/identifier login rate limiter. Disabled/ineligible users receive a generic PIN credential failure, while a known eligible user without a PIN is guided to password login for enrollment.
- Keep PIN management authenticated: setup/change/reset/disable never return credential material; change accepts current PIN or password, and reset/disable require the password. Public PIN login returns the same generic credential failure for unknown, ineligible, disabled, or unconfigured accounts to prevent account/enrollment enumeration; Flutter always offers a neutral password fallback. No device-registration layer is introduced in this phase.

## 2026-09-15: P1 request cancellation

- Use the existing native `fetch` client with an optional `ApiRequestOptions.signal`; do not add a networking library or change backend contracts. Abort errors remain distinguishable from real `ApiError` failures so cancellation does not create UI errors or retry loops.
...
## 2026-09-16: TokCounter TikTok Public Daily Collector & Metric Integrity

- **Authoritative KPI & Synthetic Metric Disclosure**: Follower counts from the TokCounter DOM are exact integers. TokCounter client-side JavaScript artificially manipulates Like counts (module 10514 bumps like counts between 10,050 and 1,049,000 by +50); synthetic Likes must never be represented as authoritative TikTok data.
- **Relational Deduplication Architecture**: `TikTokPublicAccount` holds deduplicated accounts (`username` unique). Multiple retail stores (such as Store 27837 and Store 27368 sharing `@o_taweekitburiram`) link to a single `TikTokPublicAccount` via `StoreMaster.tiktokPublicAccountId`, eliminating duplicate metric rows. Daily snapshots are stored in `TikTokPublicDailyMetric` with a composite unique constraint `[tiktokPublicAccountId, metricDate]`.
- **Collector Resilience & Rate Limiting**: The local headless Chromium collector intercepts and aborts unneeded third-party advertising, trackers, and background boosted accounts (`azhar_baby_`, `tanviraa..1`) to prevent IP rate-limiting on `tiktok-api.tokcounter.com`. A polite 5-second pacing delay and idempotent date resumption prevent unnecessary calls.
- **Railway Cron Coexistence**: The existing Railway `tiktok-daily-metrics-cron` handles private OAuth-connected accounts, while the local `com.oppo.tiktok-public-daily-collector` launchd daemon at 01:30 Asia/Bangkok handles unauthenticated public store profiles. Railway cron must remain enabled.

## 2026-09-18: Black app outbound reply state transition

- Treat successful authenticated staff outbound delivery as one canonical domain transition across text, image, video, LINE OA Manager PDF, shared PDF, and retry/idempotency paths. The transition is executed inside the same Prisma transaction as the outbound message/media persistence and is awaited before returning success.
- Persist `bmReplyStatus=REPLIED`, `followUpStatus=COMPLETED`, and `latestMessageAt` together with an activity actor record. Use `ownerUserId IS NULL` as the only assignment condition so legacy conversations are assignable on their first staff reply while existing owners remain unchanged. Do not add `repliedAt` or `repliedByUserId` because the current schema has no such fields; `latestMessageAt`, `Message.sentAt`, and the activity actor are the established equivalents.
- Build outbound realtime events from the post-send state (`REPLIED` plus the effective owner), never from the pre-send conversation snapshot. Keep Flutter’s local optimistic state and add a canonical detail refresh after success as a convergence fallback; do not infer persisted reply status from UI state.
- Keep inbound webhook reset logic authoritative for customer messages (`REPLIED` → `NOT_REPLIED`) and keep system/bot messages outside the staff transition. No schema migration or production data backfill is necessary.
