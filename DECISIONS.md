...
## 2026-09-09: Batch LINE nickname mapping behind the existing profile coordinator

- Use a small in-process recent-chat snapshot keyed by `sessionKey`, OA ID, and bot ID. Concurrent refreshes share one bounded five-page/125-chat discovery; a ready snapshot lives for 60 seconds and failed discovery is briefly cached for 30 seconds. This removes per-job repeated scans without introducing a queue schema or persistent mapping state.
- Run background refreshes as non-blocking `RECENT_RESOLUTION` operations. Existing customer text/image relay stays `MANUAL_DIAGNOSTIC` priority and continues to wait briefly when needed; background mapping defers on contention, and all browser work remains under the existing local mutex/database lease, so one persistent profile cannot receive duplicate Chromium operations.
